<table class="table table-striped table-condensed">
    ...
    @foreach($pedido->items_entrega as $item)
        <tr>
            ...
            <td id="td{{$loop->index}}" data-content="0"><input name='txtItem[]' id="txtItem{{$loop->index}}" type='text' class='form-control input-hg-12 hidden items-txt text-uppercase'>{{ Form::select('item_id[]', array(null), null, ['class'=>'items-select2','required', 'id'=>'item_id'.$loop->index.'', 'onchange'=>'javascript:cambiarUnidad('.$loop->index.');']) }}</td>
