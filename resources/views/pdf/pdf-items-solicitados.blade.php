...
@section('content')
    ...
    <table class="table table-bordered table-condensed">
        <thead>
        <tr>
            <th colspan="5" style="background-color: #c0e674;">LISTADO DE ITEMS SOLICITADOS</th>
        </tr>
        <tr>
            <th width="4%;">Item</th>
        ...
        <!-- ITEMS TEMPORALES -->
        @php $aux=1;@endphp
        @foreach($pedido->items_temporales as $item_temporal)
            <tr>
                <th scope="row">{{$aux}}</th>
                <td>{{$item_temporal->nombre}}</td>
                <td>{{$item_temporal->pivot->cantidad}}</td>
                <td>{{$item_temporal->unidad->nombre}}</td>
                <td>TEMPORAL</td>
            </tr>
            @php $aux++;@endphp
        @endforeach
